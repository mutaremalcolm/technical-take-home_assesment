...
const IdeaCard: React.FC<IdeaCardProps> = ({ idea, onDelete, onSave }) => {
  const [charCount, setCharCount] = useState(idea.content ? idea.content.length : 0);
    ...
    className={` form relative  bg-opacity-50 rounded-lg shadow-md border border-clearScoreGrey 
      ...
      >
       <div className="flex justify-between items-center mt-0 mb-2">
          ...
          Updated: {idea.createdTime ? formatDateTime(idea.createdTime) : 'N/A'}<br/>
          Created: {idea.updatedTime ? formatDateTime(idea.updatedTime) : 'N/A'}
          </p>
          </div>
      <div>
        <input
